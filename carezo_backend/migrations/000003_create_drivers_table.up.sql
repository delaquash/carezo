...
CREATE TABLE IF NOT EXISTS drivers (
    ...
    status VARCHAR(20) DEFAULT 'active'
        CHECK (status IN ('active', 'on_trip', 'unavailable', 'suspended')),
...
CREATE INDEX idx_drivers_gender 
ON drivers(gender);

CREATE INDEX idx_drivers_rating 
ON drivers(average_rating DESC);

-- ✅ Partial UNIQUE (recommended for real systems)
CREATE UNIQUE INDEX idx_drivers_license_unique
